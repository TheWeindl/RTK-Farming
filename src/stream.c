...
typedef struct {
    dev_t dev;              /* pipe control type */
    int state;              /* state (0:close,1:wait,2:connect) */
#ifdef WIN32
    HANDLE thread;
#else
    struct sockaddr_un address;
    dev_t client;           /* client pipe */
#endif
} ipc_t;
...
#ifdef WIN32
static DWORD WINAPI ConnectToNewClient(void *arg)
{
    boolean isConnected = FALSE;
    ipc_t *ipc = (ipc_t*) arg;

    trace(3, "IPC: Waiting for new connection\n");

    while(ipc->state != 0)
    {
        if(ipc->state != 2)
        {
            isConnected = ConnectNamedPipe(ipc->dev,0) != 0;

            if(isConnected || GetLastError() == ERROR_PIPE_CONNECTED)
            {
                trace(3, "IPC: Client connected\n");
                ipc->state = 2;
            }
        }
    }
...
static int stateipc(ipc_t *ipc)
{
    return ipc?ipc->state:0;
}

static ipc_t* openipc(const char *path, char *msg)
{
    ipc_t* ipc;
    char name[128];
#ifdef WIN32
    ...
    tracet(3,"openipc: path=%s msg=%s\n",path,msg);

    memset(name,0,128);

    if(!(ipc=(ipc_t*)calloc(1, sizeof(ipc_t)))) return NULL;
    if((strstr(path, pn)))
        strncpy(name, path, 128);
    else {
        strncpy(name, pn, 128);
        strncpy(name+strlen(pn), path, 128 - strlen(pn));
    }

    trace(3, "ipc path: %s\n", name);

#ifdef WIN32
    ipc->dev = CreateNamedPipe(name, PIPE_ACCESS_OUTBOUND, PIPE_TYPE_BYTE | PIPE_WAIT,
                               PIPE_UNLIMITED_INSTANCES, buffsize, 0, NMPWAIT_USE_DEFAULT_WAIT, NULL);
    if(ipc->dev == INVALID_HANDLE_VALUE)
    {
        trace(2,"IPC error CreateNamedPipe\n");
        free(ipc);
        return NULL;
    }
    ipc->state = 1;
    ipc->thread = CreateThread(NULL, 0, ConnectToNewClient, ipc, 0, NULL);
#else
    ipc->state = 1;
#endif
...
static void closeipc(ipc_t* ipc)
{
    if(!ipc) return;

#ifdef WIN32
    ipc->state = 0;
    DisconnectNamedPipe(ipc->dev);
    CloseHandle(ipc->dev);
    WaitForSingleObject(ipc->thread, INFINITE);
...
static int writeipc(ipc_t *ipc, unsigned char *buff, int n, char *msg)
{
    int ns = 0;
#ifdef WIN32
    DWORD written;
...
#ifdef WIN32
    trace(3,"IPC Write to pipe, number of bytes:%i\n", n);
    if(WriteFile(ipc->dev, buff, n, &written, 0) == FALSE)
    {
        trace(2,"IPC connection los\n");
        ipc->state = 1;
    }
    trace(3,"IPC Written bytes DWORD: %i\n", written);
    ns = (int)written;
    trace(3,"IPC Written bytes int: %i\n", ns);
#else
...
*                                 STR_IPC      = inter process communication
...
extern int stropen(stream_t *stream, int type, int mode, const char *path)
{
    ...
    switch (type) {
        ...
        case STR_IPC     : stream->port=openipc   (path,     stream->msg); break;
        /*FARMING*/
...
extern void strclose(stream_t *stream)
{
    ...
    if (stream->port) {
        switch (stream->type) {
            ...
            case STR_IPC     : closeipc   ((ipc_t    *)stream->port); break;
            /*FARMING*/
        }
    }
    else {
...
extern int strwrite(stream_t *stream, unsigned char *buff, int n)
{
    ...
    switch (stream->type) {
        ...
        case STR_FTP     :
        case STR_HTTP    :
        case STR_IPC     : ns=writeipc   ((ipc_t    *)stream->port,buff,n,msg); break;
        /*FARMING*/
        default:
            strunlock(stream);
            return 0;
...
extern int strstat(stream_t *stream, char *msg)
{
    ...
    switch (stream->type) {
        ...
        case STR_IPC     : state=stateipc   ((ipc_t    *)stream->port); break;
        /*FARMING*/
        default:
            strunlock(stream);
            return 0;
